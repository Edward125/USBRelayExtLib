...
{
	m_hWindow = XWnd_CreateWindow(0, 0, 290, 430, L"USB Relay 管理器"); //main window
	if(m_hWindow)
	{
		...
		m_hFindDeviceBtn = XBtn_Create(10, 10, 90, 25, L"查找设备", m_hWindow); 
		...
		m_hOpenDeviceBtn = XBtn_Create(10, 45, 90, 25, L"打开设备", m_hWindow);
		//register the button click event
		...
		m_hOpenDeviceBtn = XBtn_Create(170, 45, 90, 25, L"关闭设备", m_hWindow);
		...
		for (int i = 0; i < 8; i++)
		{
			...
			wsprintf(relayName, L"继电器 %d", i+1);

			m_hRelayText[i] = XEdit_Create(pos_text_x, pos_y, 90, 25, m_hWindow);
			XEdit_SetText(m_hRelayText[i], relayName);

			m_hRelayOpenBtn[i] = XBtn_Create(pos_open_btn_x, pos_y, 60, 25, L"打开", m_hWindow);
			...
			m_hRelayCloseBtn[i] = XBtn_Create(pos_close_btn_x, pos_y, 60, 25, L"关闭", m_hWindow);
		...
		m_hOpenAllRelayBtn  = XBtn_Create(10, pos_y+10, 120, 50, L"打开所有", m_hWindow);
		XCGUI_RegEleEvent(m_hOpenAllRelayBtn, XE_BNCLICK, &CUSBRelayWnd::OnOpenAllRelayBtnClick);

		//8. create close-all relay button
		m_hCloseAllRelayBtn = XBtn_Create(140, pos_y+10, 120, 50, L"关闭所有", m_hWindow);
